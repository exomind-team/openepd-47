...
uint8_t* epd_hl_get_back_framebuffer(EpdiyHighlevelState* state) {
    assert(state != NULL);
    return state->back_fb;
}
...
enum EpdDrawError epd_hl_update_screen_full(
    ...
    int fb_size = col_bytes * fb_height;
    memcpy(state->back_fb, state->front_fb, fb_size);
