...
lcd_calculate_frame(RenderContext_t* ctx, int thread_id) {
    ...
    // Prefetch before starting LCD: 16-bit drains lines twice as fast as 8-bit.
    const int bus_width = epd_get_display()->bus_width;
    const int base_prefetch = 63;
    int trigger_line = int_min(base_prefetch * bus_width / 8, max_y - min_y);
